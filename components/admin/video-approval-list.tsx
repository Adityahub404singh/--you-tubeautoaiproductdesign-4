...
function Toast({ title, onClose }: { title: string; onClose: () => void }) {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex items-center gap-3 bg-green-600 text-white px-5 py-3 rounded-xl shadow-2xl">
            ...
            return (
              <Card key={video.id} className={`overflow-hidden ${video.riskLevel === "high" ? "border-red-500/50" : ""}`}>
                ...
                <CardContent className="space-y-3">
                  ...
                  {filter === "approved" && <div className="flex items-center gap-2 p-2 bg-green-500/10 rounded-md text-xs text-green-500"><CheckCircle2 className="h-3 w-3" />YouTube upload ready</div>}
                  {video.aiScore >= 85 && <div className="flex items-center gap-2 p-2 bg-green-500/10 rounded-md text-xs text-green-500"><TrendingUp className="h-3 w-3" />High viral potential</div>}
                </CardContent>
                {filter === "pending" && (
